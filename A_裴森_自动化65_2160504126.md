...
&emsp;在二维情况下，当均值为***0*** 时，高斯滤波器常常被定义为以下的形式：<br><br>
&ensp;![1](http://latex.codecogs.com/png.latex?p(x,y)=\frac{1}{2\pi\sigma^{2}}exp(-\frac{x^{2}+y^{2}}{2\sigma^{2}}))&ensp;<br><br>
&emsp;在上式中，![2](http://latex.codecogs.com/png.latex?(x,y))&ensp;代表的是滤波器中某个位置的索引，在通常情况下，以滤波器的中心位置作为![3](http://latex.codecogs.com/png.latex?(0,0))&ensp;索引，这时候，如果滤波器的维度为***奇数***，那么处理起来就会方便得多。由于二维高斯分布存在着对称性，这意味着，我们无论以哪个方向作为`x`或者`y`的正方向，只要到中心点的距离相等，这些点便具有相同的值。这种分布特性从 **图片1** 中可以清楚的看到。<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/gaussian.png width=650 height=300/><br> 
**<center>图片1.二维高斯分布</center>**<br>
&emsp;根据上述说明，不妨以矩阵中向右的方向为`x`的正方向，向下的方向为`y`的正方向，则可以获得位置坐标矩阵为：<br><br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/matrix.gif /><br><br>
&emsp;根据坐标矩阵和二维高斯分布函数，可以得到如下滤波器：<br>
* 均值***u=0***
* 标准差***sigma=1.5***
* 采用了整数归一化方法，***各个位置的权重除以矩阵左上角的元素后取整***<br><br>

&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/result.gif /><br><br>

&emsp;如果选择标准差***sigma=0.8***，那么会得到以下滤波器：<br><br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/result2.gif /><br><br>
&emsp;在接下来的高斯模糊处理中，所采用的滤波器均为标准差***sigma=1.5***<br>
***
### 二.高斯滤波与中值滤波
&emsp;高斯滤波器的原理就是在设定的滤波器范围内加权求和，是一种卷积操作。而中值滤波的原理则更加简单，它是在给定的范围内求灰度值的中位数，然后将滤波器中心位置处的灰度值用该中位数替代。这里所说的滤波器中心与第一部分中的含义相同。<br>
&emsp;利用第一部分求出的标准差***sigma=1.5***时的高斯滤波器，对图像进行卷积操作，得到结果如下图所示：<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test1_3x3_gaussian.png width=650 height=300/><br>
**<center>图片2. 3x3高斯模糊</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test1_5x5_gaussian.png width=650 height=300/><br>
**<center>图片3. 5x5高斯模糊</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test1_7x7_gaussian.png width=650 height=300/><br>
**<center>图片4. 7x7高斯模糊</center>**<br>

&emsp;上面三幅图显示了原始图像与高斯模糊后图像的对比。从图片中可以看出，当滤波器的尺寸逐渐增大时，图像的模糊程度明显增加，为了对比***高斯模糊*** 与***中值模糊*** ，将两种方法的处理结果放在一起，效果如下：<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test1_3x3.png width=650 height=300/><br>
**<center>图片5. 3x3滤波</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test1_5x5.png width=650 height=300/><br>
**<center>图片6. 5x5滤波</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test1_7x7.png width=650 height=300/><br>
**<center>图片7. 7x7滤波</center>**<br>
&emsp;从上面的几幅图可以看出，在滤波器尺寸相同的情况下，中值滤波的模糊程度要超过高斯模糊。同样地，按照上述方法对其他测试图片进行处理，得到对比结果如下：<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test2_3x3.png width=650 height=300/><br>
**<center>图片5. 3x3滤波</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test2_5x5.png width=650 height=300/><br>
**<center>图片6. 5x5滤波</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test2_7x7.png width=650 height=300/><br>
**<center>图片7. 7x7滤波</center>**<br>
...
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test3_unsharp.png width=650 height=300/><br>
**<center>图片8. Unsharp Masking</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test4_unsharp.png  width=650 height=300/><br>
**<center>图片9. Unsharp Masking</center>**<br>
&emsp;图片8和图片9中使用的高斯模糊滤波器仍然是第一问中当***sigam=1.5*** 时的***3x3*** 结构。从图中可以看出，Unsharp Masking增强了物体的边缘，当目标与背景差别较大时，效果尤为明显。<br>
...
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test3_sobel.png  width=650 height=300/><br>
**<center>图片10. Sobel</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test4_sobel.png  width=650 height=300/><br>
...
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/result3.gif /><br><br>
&emsp;类似地，用上面展示的滤波器与图像进行卷积操作，便可以得到Laplace滤波的结果，这里也要注意的是，在求的灰度值后，应该将小于0的部分置为0，浮点数要取整，从而保证图像正常显示。在进行上述操作后，可以得到以下图像：<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test3_laplace.png  width=650 height=300/><br>
**<center>图片12. Laplace</center>**<br>
&emsp;<img src= C:/Users/peisen/Desktop/数字图像处理/hw_4/test4_laplace.png  width=650 height=300/><br>
